...
@frappe.whitelist()
def get_sales_invoice_page_context(name: str) -> dict[str, object]:
	if frappe.session.user == "Guest":
		frappe.throw(_("Authentication required"), frappe.PermissionError)
	...
	sales_order_names = _linked_sales_order_names_from_sales_invoice(doc.name)
	delivery_names = _linked_delivery_names_from_sales_invoice(doc.name)
	can_read_payment_entry = _can_read_doctype("Payment Entry")
	payment_entry_reference_count = _linked_payment_entry_reference_count(doc.name)
	payment_entries = _linked_payment_entries_for_invoice(doc.name)
	return_names = _linked_return_invoice_names(doc.name)
	customer = _linked_doc("Customer", doc.get("customer"), ["name"])
	source_invoice = _linked_doc("Sales Invoice", doc.get("return_against"), ["name", "status", "posting_date", "outstanding_amount"])
	source_warehouse = _linked_doc("Warehouse", doc.get("set_warehouse"), ["name"])
	target_warehouse = _linked_doc("Warehouse", doc.get("set_target_warehouse"), ["name"])
	todos = _linked_reference_todos(
		[
			...
			("Customer", doc.get("customer")),
			("Sales Invoice", source_invoice.get("name") if source_invoice else None),
			*[("Sales Order", order_name) for order_name in sales_order_names],
			*[("Delivery Note", delivery_name) for delivery_name in delivery_names],
			*[("Payment Entry", payment.get("name")) for payment in payment_entries],
			*[("Sales Invoice", return_name) for return_name in return_names],
		]
	)
	...
	return {
		"summary": {
			...
			"settled_amount": _sales_invoice_settled_amount(doc),
			"payment_progress_percent": _sales_invoice_payment_progress_percent(doc),
			"total_qty": float(doc.total_qty or 0),
			"is_return": int(doc.is_return or 0),
			"return_against": doc.get("return_against"),
			...
			"set_target_warehouse": doc.get("set_target_warehouse"),
			"po_no": doc.get("po_no"),
			"po_date": str(doc.po_date) if doc.get("po_date") else None,
			"payment_terms_template": doc.get("payment_terms_template"),
			"select_print_heading": doc.get("select_print_heading"),
			"language": doc.get("language"),
			"sales_order_count": len(sales_order_names),
			"delivery_count": len(delivery_names),
			"payment_entry_count": len(payment_entries),
			"payment_entry_reference_count": payment_entry_reference_count,
			"return_count": len(return_names),
		},
		"linked_documents": {
			"customer": {"doctype": "Customer", **customer} if customer else None,
			"sales_orders": _doc_rows("Sales Order", sales_order_names, ["name", "status", "transaction_date", "delivery_date"], "transaction_date asc, creation asc"),
			"deliveries": _doc_rows("Delivery Note", delivery_names, ["name", "status", "posting_date"], "posting_date asc, creation asc"),
			"payment_entries": payment_entries,
			"source_invoice": source_invoice,
			"returns": _doc_rows("Sales Invoice", return_names, ["name", "status", "posting_date", "outstanding_amount"], "posting_date desc, creation desc"),
			"source_warehouse": {"doctype": "Warehouse", **source_warehouse} if source_warehouse else None,
			"target_warehouse": {"doctype": "Warehouse", **target_warehouse} if target_warehouse else None,
		},
		"support": {
			...
			"settlement_note": _sales_invoice_settlement_note(doc, delivery_names, payment_entries),
			"customer_response_hint": _sales_invoice_customer_response_hint(doc, payment_entries),
			"next_action": _sales_invoice_next_action(doc, payment_entries, return_names, todos),
			"detail_guide": _sales_invoice_detail_guide(doc),
		},
	}
...
def _linked_doc(doctype: str | None, name: str | None, fields: list[str] | None = None) -> dict[str, object] | None:
	...
	rows = _safe_get_list(doctype, filters={"name": name}, fields=field_list, limit_page_length=1)
	return rows[0] if rows else None


def _can_read_doctype(doctype: str | None) -> bool:
...
def _safe_get_list(doctype: str, **kwargs) -> list[dict[str, object]]:
	try:
...
def _linked_sales_order_names_from_sales_invoice(sales_invoice: str) -> list[str]:
	if not sales_invoice or not frappe.db.exists("DocType", "Sales Invoice Item"):
		return []
	rows = frappe.get_all(
		"Sales Invoice Item",
		...
		fields=["sales_order"],
		distinct=True,
		order_by="sales_order asc",
	)
	names = [row["sales_order"] for row in rows if row.get("sales_order")]
	if not names:
		return []
	visible = frappe.get_list(
		"Sales Order",
...
def _linked_delivery_names_from_sales_invoice(sales_invoice: str) -> list[str]:
	if not sales_invoice or not frappe.db.exists("DocType", "Sales Invoice Item"):
		return []
	rows = frappe.get_all(
		"Sales Invoice Item",
		filters={"parent": sales_invoice, "delivery_note": ["is", "set"]},
		fields=["delivery_note"],
		distinct=True,
		order_by="delivery_note asc",
	)
	names = [row["delivery_note"] for row in rows if row.get("delivery_note")]
	if not names:
		return []
	visible = frappe.get_list(
		"Delivery Note",
...
def _linked_return_invoice_names(sales_invoice: str) -> list[str]:
	if not sales_invoice:
		return []
	rows = frappe.get_list(
		"Sales Invoice",
		filters={"is_return": 1, "return_against": sales_invoice, "docstatus": ["!=", 2]},
		fields=["name"],
		order_by="posting_date desc, creation desc",
		limit_page_length=100,
	)
...
def _linked_payment_entries_for_invoice(sales_invoice: str) -> list[dict[str, object]]:
	...
	parent_names = list(dict.fromkeys(row["parent"] for row in references if row.get("parent")))
	if not parent_names:
		return []
	payments = _safe_get_list(
		"Payment Entry",
		filters={"name": ["in", parent_names], "docstatus": ["!=", 2]},
		fields=["name", "posting_date", "payment_type", "party", "mode_of_payment", "paid_amount", "received_amount"],
		order_by="posting_date asc, creation asc",
		limit_page_length=100,
	)
	index = {row["name"]: row for row in payments}
	results: list[dict[str, object]] = []
	for reference in references:
		...
		results.append({
			...
			**index[parent],
			"allocated_amount": float(reference.get("allocated_amount") or 0),
			"reference_outstanding_amount": float(reference.get("outstanding_amount") or 0),
		})
	return results
...
def _sales_invoice_settled_amount(doc) -> float:
	grand_total = float(doc.get("grand_total") or 0)
	if int(doc.get("is_return") or 0) or grand_total <= 0:
		return 0.0
	outstanding = max(float(doc.get("outstanding_amount") or 0), 0.0)
	return max(grand_total - outstanding, 0.0)


def _sales_invoice_payment_progress_percent(doc) -> int | None:
	grand_total = float(doc.get("grand_total") or 0)
	if int(doc.get("is_return") or 0) or grand_total <= 0:
		return None
	return round((_sales_invoice_settled_amount(doc) / grand_total) * 100)


def _sales_invoice_settlement_note(doc, delivery_names: list[str], payment_entries: list[dict[str, object]]) -> str:
	status = str(doc.get("status") or "").strip()
	if int(doc.get("is_return") or 0):
		if doc.get("return_against"):
			return "This invoice is a credit note against the source invoice and should be treated as a correction chain, not a normal collection document."
		return "This invoice is a credit note and should be reviewed as a correction entry."
	if status == "Paid":
		return "Settlement is fully completed on this invoice."
	if status == "Partly Paid":
		return "Partial payment is already recorded. Keep the remaining outstanding visible before confirming closure."
	if status == "Overdue":
		return "This invoice is overdue and collection follow-through should start from the open outstanding balance."
	if status == "Unpaid":
		return "No payment settlement is recorded yet on this invoice."
	if int(doc.get("update_stock") or 0) and not delivery_names:
		return "This invoice also moved stock directly, so warehouse context remains part of the execution read."
	if payment_entries:
		return "Payment entries are already linked and should be reviewed together with the invoice status."
	return "Use the linked order, delivery, and settlement records together when reading this invoice."


def _sales_invoice_customer_response_hint(doc, payment_entries: list[dict[str, object]]) -> str:
	status = str(doc.get("status") or "").strip()
	if int(doc.get("is_return") or 0):
		return "Tell the customer this document is a credit note against the earlier invoice and is being handled as a return or correction."
	if status == "Paid":
		return "Tell the customer this invoice is fully settled."
	if status == "Partly Paid":
		return "Tell the customer partial payment is already received and the remaining balance is still open."
	if status == "Overdue":
		return "Tell the customer the invoice is overdue and payment follow-through is now the main open item."
	if status == "Unpaid":
		return "Tell the customer the invoice is issued and payment is still outstanding."
	if payment_entries:
		return "Tell the customer payment records already exist on this invoice and the settlement posture is being reviewed."
	return "Tell the customer the invoice is active and the linked commercial documents remain available for follow-up."


def _sales_invoice_next_action(doc, payment_entries: list[dict[str, object]], return_names: list[str], todos: list[dict[str, object]]) -> str:
	status = str(doc.get("status") or "").strip()
	if todos:
		task = todos[0]
		task_desc = task.get("description") or task.get("name")
		return f"Work the current follow-up task first: {task_desc}."
	if int(doc.get("is_return") or 0) and doc.get("return_against"):
		return "Review the source invoice first, then confirm whether payment allocation or customer credit follow-through is still required."
	if status == "Overdue":
		return "Start collection follow-through now and use the linked payment entry context before giving customer-facing commitments."
	if status == "Unpaid":
		return "Begin payment follow-through now because the invoice is issued but no settlement is recorded yet."
	if status == "Partly Paid":
		return "Review the linked payment entries first and confirm the exact remaining balance before closing the billing conversation."
	if return_names:
		return "Review the linked return invoices first so the customer-facing billing posture stays aligned."
	if int(doc.get("update_stock") or 0):
		return "Use the line and warehouse sections below to verify direct stock movement context together with settlement posture."
	return "Use the linked order, delivery, and settlement records together when customer questions reach invoice stage."


def _sales_invoice_detail_guide(doc) -> str:
	if doc.docstatus == 0:
		return "Use the detailed form sections below to complete items, taxes, payment structure, and customer-facing terms before submitting the invoice."
	return "Use the detailed form sections below for precise settlement, correction, and line-level maintenance while keeping the invoice summary above as the main working guide."
