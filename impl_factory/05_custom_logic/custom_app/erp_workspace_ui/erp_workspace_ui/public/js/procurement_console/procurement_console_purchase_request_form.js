...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .erpw-managed-pr-page { padding-bottom: 22px; }
      .erpw-managed-pr-shell { display: grid; gap: 12px; }
      .erpw-managed-pr-card { display: grid; gap: 14px; }
      .erpw-managed-pr-grid { display: grid; grid-template-columns: repeat(3, minmax(180px, 1fr)); gap: 12px; align-items: end; }
      ...
      .erpw-managed-pr-input, .erpw-managed-pr-table input { width: 100%; min-height: 40px; border: 1px solid #d5e2ef; border-radius: 12px; padding: 0 12px; font-size: 13.5px; color: #0f172a; background: #fff; box-shadow: inset 0 1px 0 rgba(255,255,255,0.95); box-sizing: border-box; }
      ...
      .erpw-managed-pr-input[readonly] { background: #f8fafc; color: #475569; }
      .erpw-managed-pr-table-wrap { overflow-x: auto; border: 1px solid #dbe6f2; border-radius: 14px; background: #fff; }
      .erpw-managed-pr-table { width: 100%; min-width: 920px; border-collapse: collapse; table-layout: fixed; }
      .erpw-managed-pr-table th { padding: 11px 12px; font-size: 10.5px; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase; color: #64748b; text-align: left; background: #f8fafc; border-bottom: 1px solid #e5edf5; }
      .erpw-managed-pr-table td { padding: 10px 12px; vertical-align: top; border-bottom: 1px solid #eef3f8; }
      .erpw-managed-pr-table tr:last-child td { border-bottom: 0; }
      .erpw-managed-pr-table .qty { width: 120px; }
      .erpw-managed-pr-table .date { width: 170px; }
      .erpw-managed-pr-table .uom { width: 120px; }
      .erpw-managed-pr-table .row-action { width: 88px; text-align: right; }
      .erpw-managed-pr-row-button, .erpw-managed-pr-add-line { min-height: 34px; border: 1px solid #d5e2ef; border-radius: 10px; background: #fff; color: #12365f; font-weight: 740; font-size: 12px; padding: 0 11px; }
      .erpw-managed-pr-add-line { justify-self: start; }
      .erpw-managed-pr-message { min-height: 22px; font-size: 12.5px; color: #64748b; }
      ...
      @media (max-width: 900px) { .erpw-managed-pr-grid { grid-template-columns: 1fr; } .erpw-managed-pr-table { min-width: 820px; } }
    `;
    ...
    return `
      <section class="erpw-child-card erpw-managed-pr-card" data-erpw-managed-pr-form>
        <div class="erpw-child-section-heading erpw-child-section-heading-compact">
          <div class="erpw-child-section-title">Purchase Request Draft</div>
          <div class="erpw-child-section-note">Save a draft Purchase Material Request. Submission and conversions stay outside Phase 5A.</div>
        </div>
        <div class="erpw-managed-pr-grid">
          ...
          <div class="erpw-managed-pr-field"><label>Company</label><input class="erpw-managed-pr-input" data-field="company" type="text" readonly value="${escapeHtml(header.company || "")}"></div>
        </div>
            ...
            <tbody>
              ${(form.items || []).map((row, index) => `
                <tr data-row-index="${index}">
                  ...
                  <td><input data-row-field="uom" readonly value="${escapeHtml(row.uom || "")}" placeholder="Derived"></td>
  ...
  function loadingPayload() {
    return {
      ...
      summary: { kicker: "Purchase request draft", title: "New Purchase Request", subtitle: "Loading managed draft form.", chips: [{ label: "Draft" }] },
      controls: { actions: [] },
