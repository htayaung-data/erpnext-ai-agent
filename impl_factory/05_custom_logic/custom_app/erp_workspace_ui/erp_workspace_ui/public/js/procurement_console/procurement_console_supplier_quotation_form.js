...
(function () {
  ...
  function actionConfig(payload, viewState) {
    ...
    return actions.map((action) => Object.assign({}, action, {
      ...
      icon: action.key === "save_supplier_quotation" ? "check" : action.key === "back_to_supplier_quotations" ? "arrow-left" : "refresh",
      handler() {
  ...
  function renderPayload(viewState, overridePayload) {
    ...
    const extra = payload.state && payload.state.kind && payload.state.kind !== "ready" && payload.state.kind !== "loading" ? renderState(payload) : formMarkup(form) + readinessCardMarkup(payload);
