...
(function () {
  ...
  function actionConfig(payload, viewState) {
    ...
    return actions.map((action) => Object.assign({}, action, {
      ...
      icon: action.key === "save_rfq" ? "check" : action.key === "back_to_rfqs" ? "arrow-left" : "refresh",
      handler() {
  ...
  function cleanupSuggestionsOnly() {
  ...
  function showSuggestions($shell, input, doctype, viewState) {
    ...
    frappe.call({ method: "frappe.desk.search.search_link", args: { doctype, txt, page_length: 8 } }).then((response) => {
      ...
      if (requestId !== suggestionRequestId || !rows.length || !document.body.contains(input)) return;
      cleanupSuggestionsOnly();
  ...
  function renderPayload(viewState, overridePayload) {
    ...
    const extra = payload.state && payload.state.kind && payload.state.kind !== "ready" && payload.state.kind !== "loading" ? renderState(viewState.$shell, payload) : formMarkup(form) + readinessCardMarkup(payload) + output;
