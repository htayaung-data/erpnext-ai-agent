...
- Warehouse Console W11/W11A is docs-only; the accepted next direction is a W11B docs-only Purchase Receipt receiving design package. No Warehouse execution implementation phase should start until Warehouse Agent, Security/Stability Review Agent, Operation Reviewer Agent, Main Control, and the owner explicitly approve a scoped implementation.
