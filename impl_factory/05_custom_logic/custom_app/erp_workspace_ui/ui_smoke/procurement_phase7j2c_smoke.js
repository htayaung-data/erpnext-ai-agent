...
const TIMEOUT = Number(process.env.ERPW_PROCUREMENT_PHASE7J2C_TIMEOUT || 60000);
const ARTIFACT_DIR = process.env.ERPW_PROCUREMENT_PHASE7J2C_ARTIFACT_DIR || path.join(
  fs.existsSync("/freeze-artifacts") ? "/freeze-artifacts" : path.join(__dirname, "artifacts"),
  `procurement-phase7j2c-${new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z")}`
);
const ASSET_OVERRIDE_ROOT = process.env.ERPW_PROCUREMENT_PHASE7J2C_ASSET_ROOT || process.env.ERPW_PROCUREMENT_PHASE7J2A_ASSET_ROOT || "";
...
async function pageState(page, type) {
  ...
  const state = await page.evaluate((detailType) => {
    ...
    return {
      ...
      objectBriefCount: document.querySelectorAll("[data-erpw-object-profile-brief]").length,
...
function assertExactTabs(actual, expected, label) {
  const normalizedActual = actual.map((value) => String(value || "").trim()).filter(Boolean);
  assert(
    normalizedActual.length === expected.length && expected.every((tab, index) => normalizedActual[index] === tab),
    `${label}: unexpected tab set`,
    { actual: normalizedActual, expected }
  );
}
...
async function assertSupplierDetail(page, user, supplier, viewport) {
  ...
  assert(state.activeTab === "Profile", `${user.key} supplier ${viewport.key}: Profile should be default tab`, state);
  assertExactTabs(state.tabLabels, ["Profile", "Orders", "RFQs", "Quotations"], `${user.key} supplier ${viewport.key}`);
  assert(state.objectBriefCount === 0, `${user.key} supplier ${viewport.key}: duplicate below-header object brief badges should be removed`, state);
  assert(/Supplier Buying Profile/i.test(state.activePanelText), `${user.key} supplier ${viewport.key}: Profile tab should show Supplier Buying Profile`, state);
  assert(/Readiness Review/i.test(state.bodyText), `${user.key} supplier ${viewport.key}: Profile tab should include readiness guidance`, state);
  assert(/Buying contacts/i.test(state.activePanelText), `${user.key} supplier ${viewport.key}: Profile tab should include buying contacts or reference context`, state);
  assert(!/Activity|Readiness Guidance|References/i.test(state.tabLabels.join(" ")), `${user.key} supplier ${viewport.key}: removed tabs still visible`, state);
  ...
  state = await assertClean(page, "supplier", `${user.key}-supplier-${viewport.key}-orders`);
  assert(/Open or overdue purchase orders|Recent purchase orders/i.test(state.activePanelText), `${user.key} supplier ${viewport.key}: Orders tab should show order posture`, state);
  const ordersScreenshot = await capture(page, `${user.key}-supplier-${viewport.key}-orders`);
  await clickTab(page, "RFQs");
  state = await assertClean(page, "supplier", `${user.key}-supplier-${viewport.key}-rfqs`);
  assert(/RFQs/i.test(state.activePanelText), `${user.key} supplier ${viewport.key}: RFQs tab should show RFQ context`, state);
  const rfqsScreenshot = await capture(page, `${user.key}-supplier-${viewport.key}-rfqs`);
  await clickTab(page, "Quotations");
  state = await assertClean(page, "supplier", `${user.key}-supplier-${viewport.key}-quotations`);
  assert(/Supplier quotations/i.test(state.activePanelText), `${user.key} supplier ${viewport.key}: Quotations tab should show quotation context`, state);
  const quotationsScreenshot = await capture(page, `${user.key}-supplier-${viewport.key}-quotations`);
  return { defaultScreenshot, ordersScreenshot, rfqsScreenshot, quotationsScreenshot, reveal, state };
}
...
async function assertItemDetail(page, user, itemCode, viewport) {
  ...
  assert(state.activeTab === "Profile", `${user.key} item ${viewport.key}: Profile should be default tab`, state);
  assertExactTabs(state.tabLabels, ["Profile", "Suppliers & Prices", "Orders", "Quotation History"], `${user.key} item ${viewport.key}`);
  assert(state.objectBriefCount === 0, `${user.key} item ${viewport.key}: duplicate below-header object brief badges should be removed`, state);
  assert(/Item Buying Context/i.test(state.activePanelText), `${user.key} item ${viewport.key}: Profile tab should show Item Buying Context`, state);
  assert(/Readiness Review/i.test(state.bodyText), `${user.key} item ${viewport.key}: Profile tab should include readiness guidance`, state);
  ...
  assert(!/Readiness Guidance|References|Demand & Orders/i.test(state.tabLabels.join(" ")), `${user.key} item ${viewport.key}: removed or premature tabs still visible`, state);
  ...
  await clickTab(page, "Suppliers & Prices");
  state = await assertClean(page, "item", `${user.key}-item-${viewport.key}-suppliers-prices`);
  ...
  const suppliersPricesScreenshot = await capture(page, `${user.key}-item-${viewport.key}-suppliers-prices`);
  await clickTab(page, "Orders");
  state = await assertClean(page, "item", `${user.key}-item-${viewport.key}-orders`);
  assert(/Open purchase orders/i.test(state.activePanelText), `${user.key} item ${viewport.key}: Orders tab should show buying movement context`, state);
  const ordersScreenshot = await capture(page, `${user.key}-item-${viewport.key}-orders`);
  await clickTab(page, "Quotation History");
  state = await assertClean(page, "item", `${user.key}-item-${viewport.key}-quotation-history`);
  assert(/Recent supplier quotations/i.test(state.activePanelText), `${user.key} item ${viewport.key}: Quotation History tab should show quotation context`, state);
  const quotationHistoryScreenshot = await capture(page, `${user.key}-item-${viewport.key}-quotation-history`);
  return { defaultScreenshot, suppliersPricesScreenshot, ordersScreenshot, quotationHistoryScreenshot, reveal, state };
}
...
(async () => {
  assert(USERS.length >= 2, "Purchase Manager and Purchase User credentials are required for Phase 7J2C smoke");
  ...
  try {
    ...
    fs.writeFileSync(path.join(ARTIFACT_DIR, "phase7j2c-summary.json"), JSON.stringify(summary, null, 2));
    console.log(JSON.stringify(summary, null, 2));
  } catch (error) {
    const failure = { status: "fail", artifactDir: ARTIFACT_DIR, error: error.message, details: error.details || {} };
    fs.writeFileSync(path.join(ARTIFACT_DIR, "phase7j2c-summary.json"), JSON.stringify(failure, null, 2));
