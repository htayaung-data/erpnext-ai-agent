  ...
  return [
    {
      ...
      route_targets: { exception_review: { route: "warehouse-console-stock-exception", context_token: REVIEW_TOKEN }, picking: { route: "warehouse-console-picking", sales_order: "SO-REVIEW" }, receiving: { route: "warehouse-console-receiving", purchase_order: "PO-SOON" } },
    },
    {
      key: "SO-AGING:ITEM-220:Main - M",
      context_token: AGING_REVIEW_TOKEN,
      ...
      route_targets: { exception_review: { route: "warehouse-console-stock-exception", context_token: AGING_REVIEW_TOKEN }, picking: { route: "warehouse-console-picking", sales_order: "SO-AGING" } },
    },
  ];
}
...
function stockExceptionReviewPayload(contextToken = REVIEW_TOKEN) {
  return {
    workspace: workspacePayload(),
    context: sidebarPayload().context,
    state: { kind: "ready", title: "Warehouse Console ready", detail: "Stock visibility and warehouse posture are available for review." },
    ...
    panels: {
      ...
      stock: { title: "Stock Posture", summary: "Current visible stock compared with pending demand.", items: [{ label: "Item", value: "ITEM-105 Power Bank" }, { label: "Warehouse", value: "Short - M" }, { label: "Available", value: "2" }, { label: "Projected", value: "2" }, { label: "Short Qty", value: "6 Nos" }] },
      ...
      next_reviews: {
        ...
        summary: "Read-only review paths available for this exception.",
        items: [
          ...
          { label: "Stock Exceptions", value: "Return to the exception queue.", target: { route: "warehouse-console-worklist", queue_key: "stock_exceptions" } },
        ],
      },
    },
    related_rows: [
...
async function installSourceOverrides(context, diagnostics) {
  ...
  await context.route("**/api/method/frappe.desk.desk_page.getpage**", async (route) => {
    ...
    const file = isStockExceptionReview
    ...
    const name = isStockExceptionReview ? "warehouse-console-stock-exception" : isReceiving ? "warehouse-console-receiving" : isPicking ? "warehouse-console-picking" : isWorklist ? "warehouse-console-worklist" : "warehouse-console";
...
async function snapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      hasExportedStockReviewRenderer: Boolean(window.erpWorkspaceWarehouseConsole && typeof window.erpWorkspaceWarehouseConsole.renderStockExceptionReview === "function"),
    };
  });
}
...
async function exerciseUser(browser, user) {
  ...
  try {
    let sawStockExceptionReviewRenderer = false;
    let sawStockExceptionReviewServiceCall = false;
    ...
    if (hasStockRows) {
      assert(state.reviewRouteButtonCount >= 1, "Stock exception review route button did not render", { user: user.key, state });
    ...
    if (hasStockRows) {
      ...
      state = await snapshot(page);
      sawStockExceptionReviewRenderer = sawStockExceptionReviewRenderer
        || (state.diagnostics || {}).renderStockExceptionReviewEntered >= 1
        || state.stockReviewShellCount === 1;
      sawStockExceptionReviewServiceCall = sawStockExceptionReviewServiceCall
      ...
      assert(state.stockReviewRelatedCount >= 1, "Stock exception related review rows did not render", { user: user.key, state });
      assert(/warehouse-console-stock-exception/.test(state.url), "Stock exception drilldown did not stay inside Warehouse route", { user: user.key, state });
      await capture(page, `${user.key}-stock-exception-review`);
      ...
      state = await snapshot(page);
      assertClean(state, `${user.key}:review-picking`);
      assert(/warehouse-console-picking/.test(state.url), "Picking review link did not stay inside Warehouse route", { user: user.key, state });

      await page.goBack({ waitUntil: "domcontentloaded", timeout: TIMEOUT });
      await waitForStockExceptionReview(page);

      await page.locator('[data-warehouse-stock-exception-next-target="receiving"], [data-warehouse-stock-exception-related-target="receiving"]').first().click();
      ...
      state = await snapshot(page);
      assertClean(state, `${user.key}:review-receiving`);
      assert(/warehouse-console-receiving/.test(state.url), "Inbound review link did not stay inside Warehouse route", { user: user.key, state });

      await page.goBack({ waitUntil: "domcontentloaded", timeout: TIMEOUT });
      await waitForStockExceptionReview(page);
      await page.locator("[data-warehouse-stock-exception-back]").click();
    ...
    if (ASSET_ROOT) {
      ...
      assert(diagnostics.overrideHits.some((hit) => hit.key === "warehouse-stock-exception-review"), "Warehouse stock exception review source override was not used", { user: user.key, diagnostics });
    }
    assert(!diagnostics.consoleErrors.some((entry) => entry.type === "error"), "Warehouse W6B smoke recorded console errors", { user: user.key, diagnostics });
    assert(diagnostics.pageErrors.length === 0, "Warehouse W6B smoke recorded page errors", { user: user.key, diagnostics });
    assert(diagnostics.failedResponses.length === 0, "Warehouse W6B smoke recorded failed responses", { user: user.key, diagnostics });
    await context.close();
