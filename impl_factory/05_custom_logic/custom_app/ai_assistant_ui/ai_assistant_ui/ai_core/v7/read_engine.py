...
from ai_assistant_ui.ai_core.ontology_normalization import infer_output_flags, infer_write_request
...
def _normalize_yes_no_choice(message: str) -> str:
    msg = str(message or "").strip().lower()
    if not msg:
        return ""
    yes_words = {"yes", "y", "ok", "okay", "sure", "proceed", "continue", "do it"}
    no_words = {"no", "n", "cancel", "stop", "don't", "do not", "nope"}
    if msg in yes_words:
        return "yes"
    if msg in no_words:
        return "no"
...
def _match_option_choice(message: str, options: List[str]) -> str:
    ...
    msg_lc = msg.lower()
    if {x.lower() for x in normalized} == {"yes", "no"}:
        yn = _normalize_yes_no_choice(msg)
        if yn:
            return yn

    m_idx = re.search(r"\b(\d{1,2})\b", msg_lc)
    if m_idx:
        try:
    ...
    for opt in normalized:
        opt_lc = opt.lower()
        if opt_lc and (opt_lc in msg_lc or msg_lc in opt_lc):
            return opt
    return ""


def _looks_like_new_business_request(message: str) -> bool:
    msg = str(message or "").strip()
    if not msg:
        return False
    if re.search(r"\b[A-Z]{2,}-[A-Z0-9]+-\d{4}-\d+\b", msg):
        return True
    words = [w for w in re.findall(r"[A-Za-z0-9_]+", msg) if w]
    word_count = len(words)
    if word_count >= 6:
        return True
    if msg.endswith("?") and word_count >= 3:
        return True
    # Short replies are usually pending answers, not new requests.
    if word_count <= 2:
        return False
    # Mid-length free text without clear option matching is treated as a new request.
    return True


def _prepare_resume_from_pending(*, message: str, pending: Dict[str, Any]) -> Dict[str, Any]:
    """
    Handles blocker-only follow-up continuations in a deterministic way:
    - need_filters with entity options/value
    - planner_clarify with yes/no
    ...
    if mode == "planner_clarify" and options and {o.lower() for o in options} == {"yes", "no"}:
        chosen = _match_option_choice(raw_input, options)
        if not chosen:
            if _looks_like_new_business_request(raw_input):
                return {
                    "active": True,
        ...
        if chosen.lower() == "no":
            return {
                ...
                "payload": {
                    "type": "text",
                    "text": "Understood. Please restate the request with the exact report target or required filters.",
    ...
    if mode == "need_filters":
        ...
        if options:
            ...
            if not selected_value:
                ...
                text = "Please choose one of the listed options."
                return {
                    "active": False,
                    "payload": {
                        "type": "text",
...
def execute_unified_read_turn(
    *,
    ...
    if mode == "continue" and pending:
        pending_resume = _prepare_resume_from_pending(message=message, pending=pending)
    ...
    if bool(clarify_decision.get("should_clarify")):
        ...
        if reason_lc in ("missing_required_filter_value", "hard_constraint_not_supported", "entity_no_match", "entity_ambiguous"):
    ...
    for step_no in range(1, max(1, int(max_steps)) + 1):
        ...
        failed_ids = [str(x or "") for x in list(quality.get("failed_check_ids") or [])]
        data_shape_repairable = any(
        ...
        if (
            ...
            and data_shape_repairable
            and (mode != "continue")
            and str(spec_obj.get("intent") or "").strip().upper() != "TRANSFORM_LAST"
            and (candidate_cursor + 1 < len(candidate_reports))
            and (candidate_switch_attempts < 4)
        ):
            candidate_switch_attempts += 1
            candidate_cursor += 1
            selected_report = candidate_reports[candidate_cursor]
    ...
    if quality.get("verdict") == VERDICT_REPAIRABLE_FAIL:
        if str(spec_obj.get("intent") or "").strip().upper() == "TRANSFORM_LAST":
            ...
            if any(
                ...
                payload = {
                    "type": "text",
                    "text": f"{unsupported} This report path does not support the exact request yet. Should I switch to a close available report?",
                    "_pending_state": {
                        "mode": "planner_clarify",
                        "base_question": str(message or "").strip(),
                        "report_name": str(selected_report or "").strip(),
                        "filters_so_far": dict(spec_obj.get("filters") or {}) if isinstance(spec_obj.get("filters"), dict) else {},
                        "clarification_question": f"{unsupported} This report path does not support the exact request yet. Should I switch to a close available report?",
                        "clarification_options": ["yes", "no"],
                        "options": ["yes", "no"],
                        "clarification_round": 1,
                    },
