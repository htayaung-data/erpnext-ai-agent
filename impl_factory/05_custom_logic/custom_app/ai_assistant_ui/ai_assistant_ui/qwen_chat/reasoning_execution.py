	...
	*,
	...
	context = {
		...
		"evidence_catalog": evidence_catalog,
			...
			"artifact_filters": dict(family_artifact.get("filters") or {}) if isinstance(family_artifact.get("filters"), dict) else {},
			"artifact_period": dict(family_artifact.get("period") or {}) if isinstance(family_artifact.get("period"), dict) else {},
...
def _build_source_detail_drilldown_payload(
	*,
	context: Dict[str, Any],
	...
	lines: List[str] = [
		...
		"",
		"| Source document | Net COGS impact | Share of line |",
		"| --- | ---: | ---: |",
	]
	...
	return {
		...
		"reason": "The answer executed a registered governed source-detail report for the focused financial statement line.",
...
def _build_prior_grounded_row_detail_payload(
	*,
	...
	if not row_label:
		return {}
	source_detail_payload = _build_source_detail_drilldown_payload(
		context=context,
		focused_row=row,
		user_id=user_id,
	)
	if source_detail_payload:
		return source_detail_payload
