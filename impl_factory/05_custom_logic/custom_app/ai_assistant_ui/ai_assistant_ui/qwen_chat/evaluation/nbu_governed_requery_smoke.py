...
def run_nbu_governed_requery_smoke() -> Dict[str, Any]:
	...
	try:
		...
		if not any(marker in supplier_detail_lower for marker in ("profile", "payment terms", "invoice count", "recent purchase")):
			raise RuntimeError(f"Broad supplier detail follow-up returned only narrow row evidence: {supplier_detail_text[:240]}")
