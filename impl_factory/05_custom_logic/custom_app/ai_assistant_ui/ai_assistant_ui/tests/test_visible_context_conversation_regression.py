...
class VisibleContextConversationRegressionTests(unittest.TestCase):
	...
	def test_business_intent_boundaries_are_enforced_from_visible_evidence(self):
		...
		self.assertIn("can't recommend a business action from the current table alone", recommendation.answer)
		self.assertIn("Visible facts for 35th Street Mobile Wholesale", recommendation.answer)
		...
		self.assertIn("can't attribute cause from this point-in-time table alone", causal.answer)
		self.assertIn("trend or event-history evidence", causal.answer)
